...
@section('content')
    ...
    <div class="find-provider-page">
        <div class="site-header">
            <div class="left-head">
                <a href="{{ route('website.find-cleaner') }}" class="back-btn" id="backBtn"><i
                        class="ti ti-arrow-left"></i>Back to Results</a>
            </div>
        ...
        <div class="container school-profile-section-main-wrapper">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <div class="col-12">
                                            ...
                                            <textarea class="form-control" id="inquiry_message" name="message" rows="4"
                                                placeholder="Tell the cleaner about your needs, preferred start date, or any specific requirements..." required>Hi, I'm interested in learning more about your services. Could you please provide more information about availability and pricing?</textarea>
                                        </div>
                                        <div class="col-12">
                                            <div class="form-check">
                                                <input class="form-check-input" type="checkbox" id="inquiry_newsletter"
                                                    name="newsletter">
                                ...
                                <div class="card-body p-4">
                                            ...
                                            <div class="event-item border-bottom pb-3 mb-3">
                                                <div class="row align-items-center">
                                                    ...
                                                    <div class="col-md-6">
                                                        ...
                                                        <p class="event-subtitle text-muted mb-1">{{ $event->subtitle }}
                                                        </p>
                                                    ...
                                                    <div class="col-md-4 text-end">
                                                        @if ($event->cost > 0)
                                                            <div class="event-cost mb-2">
                                                                <span
                                                                    class="fw-bold text-success">${{ number_format($event->cost, 2) }}</span>
                                                            </div>
                                                        @else
                                                            <div class="event-cost mb-2">
                                    ...
                                    @if ($providerEvents->count() > 3)
                                        <div class="text-center mt-3">
                                            <a href="{{ route('provider.events', $provider->id) }}"
                ...
                <div class="col-lg-4">
                    <div class="school-information-detail-card">
                        ...
                        <div class="school-action-info-parent">
                            ...
                            @if ($provider->price_amount)
                                <div class="school-action-info">
                                    ...
                                    <div class="d-flex flex-column">
                                        <p>${{ number_format($provider->price_amount, 0) }}{{ $provider->pricing_description ? ' - ' . $provider->pricing_description : '' }}
                                        </p>
                                    </div>
                                </div>
                            @endif
                        ...
                        <div class="school-action-buttons-row">
                            @if ($provider->phone || $provider->phone_number)
                                <button class="school-action-btn-call"
                                    onclick="makePhoneCall('{{ $provider->phone ?? $provider->phone_number }}')">
                        ...
                        <div class="school-lower-action-buttons">
                            <button class="save-provider-btn {{ $provider->isSavedByUser() ? 'saved' : '' }}"
                                data-provider-id="{{ $provider->id }}">
                                <i class="ti ti-bookmark me-1"></i>
                                <span>{{ $provider->isSavedByUser() ? 'Saved' : 'Save' }}</span>
                            </button>

                            <button class="follow-provider-btn {{ $provider->isFollowedByUser() ? 'following' : '' }}"
                                data-provider-id="{{ $provider->id }}">
            ...
            <ul class="nav nav-pills school-tab-navigation-container" id="schoolTabs" role="tablist">
                ...
                @if ($provider->start_time && $provider->end_time)
                    <li class="nav-item">
                        <button class="nav-link" id="hours-tab" data-bs-toggle="pill" data-bs-target="#hours"
                            type="button">Hours</button>
                    </li>
                @endif
                ...
                @if (!empty($amenities))
                    <li class="nav-item">
                        <button class="nav-link" id="amenities-tab" data-bs-toggle="pill" data-bs-target="#amenities"
                            type="button">Amenities</button>
                    </li>
                @endif

                @if ($providerEvents->count() > 0)
                    <li class="nav-item">
                        <button class="nav-link" id="events-tab" data-bs-toggle="pill" data-bs-target="#events"
                    ...
                    @php
                        ...
                        if (isset($serviceCategories)) {
                            if ($serviceCategories instanceof \Illuminate\Support\Collection) {
                                ...
                                $serviceCategoryNames = array_map(function ($c) {
                                    return is_object($c) ? $c->name ?? '' : $c;
                                }, $serviceCategories);
                            }
                        }
                                    ...
                                    @if ($provider->price_amount)
                                        <div class="p-4 rounded-3 mb-4" style="background:#fff;border: 1px solid #ddd;">
                                            ...
                                            <h2 class="fw-bold text-success mb-1">
                                                ${{ number_format($provider->price_amount, 0) }}{{ $provider->pricing_description ? ' - ' . $provider->pricing_description : '' }}
                                            </h2>
                        ...
                        @if ($reviewCount > 0)
                            <div class="row align-items-center">
                                <div class="col-md-3 text-center">
                                    <div class="long-keyword-rating-score-big">{{ number_format($averageRating, 1) }}
                                    </div>
                                    <div>
                                        @php
                                        ...
                                        <div class="d-flex align-items-center mb-2">
                                            ...
                                            <div class="flex-grow-1 long-keyword-progress-bar">
                                                <div class="long-keyword-progress-bar-fill"
                                                    style="width:{{ ($ratingDistribution[$i] / $reviewCount) * 100 }}%; height:6px;">
                                                </div>
                                            </div>
                            ...
                            @foreach ($reviews as $review)
                                <div class="long-keyword-review-box">
                                    <h6><i class="bi bi-person-circle me-1"></i>
                                        {{ $review->user->first_name ?? 'Anonymous' }} <small class="text-muted">•
                                            {{ $review->created_at->diffForHumans() }}</small></h6>
                        ...
                        <div class="long-keyword-hours-box">
                            ...
                            <div class="row mt-4">
                                @foreach ($daysOfWeek as $day)
                                    ...
                                    <div
                                        class="col-6 {{ in_array($day, $availableDays) ? 'text-success' : 'text-muted' }}">
                ...
                <form id="inquiryMultiForm">
                    ...
                    <div class="modal-body">
                        <div class="inquiry-steps">
                            ...
                            <div class="d-flex justify-content-center mb-4 gap-3">
                                <button type="button" class="btn btn-sm btn-outline-secondary step-indicator active"
                                    data-step="0">1 Choose Type</button>
                                <button type="button" class="btn btn-sm btn-outline-secondary step-indicator"
                                    data-step="1">2 Services & Schedule</button>
                                <button type="button" class="btn btn-sm btn-outline-secondary step-indicator"
                                    data-step="2">3 Checkout</button>
                            </div>
                            ...
                            <div class="inquiry-step" id="inquiry-step-0">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-6">
                                        <div class="card p-3 type-option" data-type="carpet-upholstery"
                                    ...
                                    <div class="col-md-6">
                                        <div class="card p-3 type-option" data-type="end-tenancy"
                                            style="cursor:pointer;">
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-12">
                                        ...
                                        <textarea id="serviceNotes" class="form-control" rows="3"
                                            placeholder="Any useful notes for the cleaner (access, pets, parking)..."></textarea>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="inquiry-step" id="inquiry-step-2" style="display:none;">
                                ...
                                <div class="row g-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" id="checkout_name" class="form-control"
                                            value="{{ auth()->user() ? auth()->user()->name : '' }}" required>
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">Email *</label>
                                        <input type="email" id="checkout_email" class="form-control"
                                            value="{{ auth()->user() ? auth()->user()->email : '' }}" required>
                                    </div>
                                    <div class="col-md-6">
                    ...
                    <div class="modal-footer">
                        <div class="w-100 d-flex justify-content-between">
                            <div>
                                <button type="button" class="btn btn-secondary" id="btnBack" style="display:none;">←
                                    Back</button>
                            </div>
                            <div>
                                <button type="button" class="btn btn-outline-secondary" id="btnNext">Continue
                                    →</button>
                                <button type="button" class="btn btn-primary" id="btnSubmit" style="display:none;"><i
                                        class="ti ti-send me-1"></i> Book & Send</button>
                            </div>
                        </div>
                    </div>
                </form>
        ...
        function submitInquiry(form, submitBtnId) {
            ...
            fetch('{{ route('inquiries.store') }}', {
                ...
                .then(data => {
                    if (data.success) {
                        showInquirySuccess(
                            'Your inquiry has been sent successfully! The provider will contact you soon.');
        ...
        (function() {
            ...
            document.querySelectorAll('.type-option').forEach(card => {
                card.addEventListener('click', function() {
                    document.querySelectorAll('.type-option').forEach(c => c.classList.remove(
                        'border-primary'));
            ...
            btnNext.addEventListener('click', function() {
                ...
                if (current === 1) {
                    ...
                    const checked = Array.from(document.querySelectorAll('.service-checkbox:checked')).map(cb =>
                        JSON.parse(cb.value));
                    if (checked.length === 0) {
                    ...
                    document.getElementById('frequency_hidden').value = document.getElementById(
                    ...
                    document.getElementById('serviceNotes').value && console.log('notes:', document
                        .getElementById('serviceNotes').value);
                    buildOrderSummary();
                    showStep(2);
                    return;
                }
            ...
            function buildOrderSummary() {
                ...
                items.forEach(it => {
                    ...
                    div.innerHTML =
                        `<div>${it.label} <small class="text-muted">(${it.duration} mins)</small></div><div>€${it.price}</div>`;
                    container.appendChild(div);
                });
                const hr = document.createElement('hr');
                container.appendChild(hr);
                const freqDiv = document.createElement('div');
                freqDiv.className = 'd-flex justify-content-between';
                freqDiv.innerHTML = `<div>Frequency</div><div>${freq}</div>`;
                container.appendChild(freqDiv);
                const schedDiv = document.createElement('div');
                schedDiv.className = 'd-flex justify-content-between';
                schedDiv.innerHTML = `<div>Scheduled</div><div>${scheduled}</div>`;
                container.appendChild(schedDiv);
                if (notes) {
                    const notesDiv = document.createElement('div');
                    notesDiv.className = 'mt-2 text-muted';
            ...
            function submitMultiInquiry() {
                ...
                const originalText = submitBtn.innerHTML;
                submitBtn.disabled = true;
                submitBtn.innerHTML = '<i class="ti ti-loader me-1"></i>Booking...';
